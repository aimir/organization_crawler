...
for company in aaa:
    ...
    linked_company = reduce(lambda x, y: x + y,
                            ...
                            for r in records],
                            [])
    result[company] = linked_company

# TODO: make this fucking readable
rrr = {(a, b): len([c for c in result[a] if c.encode('ASCII', errors="replace") == b and b != a]) for a in aaa for b in AAA}
for key in list(rrr.keys())[:]:
    if rrr[key] == 0:
        rrr.pop(key)
print(rrr)
